...
4. 点击卡片，在新标签页打开网页；点击标签标题可折叠或展开其中的网页。页面顶部可以「全部折叠」「全部展开」。
5. 点击「整理网页」，通过卡片下方的「所属标签」选择框移动网页；点击卡片右上角的移除按钮并确认后删除。点击「完成整理」回到普通视图。
...
测试使用临时数据目录，验证添加、读取、删除、并发保存、旧数据迁移、标签归类、单独及全部折叠展开、重启恢复和基本请求边界，不会修改实际网页列表。
...
| `public/app.js` | 网页与标签交互、折叠展开、归类和图标回退 |
